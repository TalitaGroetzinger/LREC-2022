    ...
    * a list of sentence strs
    ...
    for _, row in dataset.iterrows():
        for filler_index in range(1, 6):
            ...
            if filler_markers:
                sent_with_filler = insert_filler_markers(sentence=row["Sentence"], filler=row[f"Filler{filler_index}"],
...
def insert_filler_markers(sentence: str, filler: str, filler_markers: Tuple[str, str]) -> str:
    ...
    try:
        ...
        if type(filler_markers) != tuple or len(filler_markers) != 2:
            raise ValueError(f"Special tokens {filler_markers} not valid. "
